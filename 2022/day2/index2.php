...
function sum2($carriage,$item) {
	if (!empty($item)) {
		$vals = array(1,2,3,"A"=>0,"B"=>1,"C"=>2,"X"=>0,"Y"=>3,"Z"=>6,);
		$result = $vals[$item[2]];
		$index = $vals[$item[0]];
		$result += $vals[$item[2]] == $vals["X"] ? $vals[($index+2)%3] : 0;
		$result += $vals[$item[2]] == $vals["Y"] ? $vals[$index] : 0;
		$result += $vals[$item[2]] == $vals["Z"] ? $vals[($index+1)%3] : 0;
		$carriage[]=$result;
	}
	return $carriage;
}
